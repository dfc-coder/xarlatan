...
EXPECTED_VERSION="${EXPECTED_VERSION:-v0.7.0}"
...
printf 'Running Xarlatan v0.7 agent-agnostic ACP Voice Gateway acceptance...\n\n'
...
ACP_BINARY="${ACP_BINARY:-$(awk '
  /^agent:[[:space:]]*$/ { in_agent=1; next }
  in_agent && /^[^[:space:]#]/ { exit }
  in_agent && /^[[:space:]]+acp:[[:space:]]*$/ { in_acp=1; next }
  in_acp && /^[[:space:]]+binary:[[:space:]]*/ {
    line=$0
...
if [[ -n "${ACP_BINARY}" && -x "${ACP_BINARY}" ]]; then
  record_pass "configured ACP executable: ${ACP_BINARY}"
else
  record_fail "configured ACP executable: ${ACP_BINARY:-not-found}"
fi
...
printf '\nSTEP 2 — ACP agent streaming + Kokoro\n'
...
if grep -Fq 'ACP response stream started' "${INTERACTION_LOG}"; then record_pass "real ACP user-facing streaming delta observed"; else record_fail "real ACP user-facing streaming delta observed"; fi
...
if [[ "${LEAK}" -eq 0 ]]; then record_pass "shutdown leaves no owned ACP/audio/inference child"; else record_fail "shutdown leaves no owned child"; fi

FINAL_RESULT=PASS
[[ "${FAILURES}" -eq 0 ]] || FINAL_RESULT=FAIL
{
  printf '# Xarlatan v0.7 Agent-agnostic ACP Voice Gateway acceptance\n\n'
  printf -- '- Timestamp UTC: `%s`\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- Host: `%s`\n' "$(hostname)"
  printf -- '- Expected version: `%s`\n' "${EXPECTED_VERSION}"
  printf -- '- ACP executable: `%s`\n' "${ACP_BINARY:-not-found}"
  ...
  printf -- '- ACP stream markers: `%s`\n' "$(grep -Fc 'ACP response stream started' "${INTERACTION_LOG}" || true)"
